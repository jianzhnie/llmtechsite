...
默认情况下，`window_size=10`，日志处理器会统计最近 10 次迭代损失、迭代时间、数据时间的均值。
...
> 备注：`log_processor` 默认输出 `by_epoch=True` 格式的日志。日志格式需要和 `train_cfg` 中的 `by_epoch` 参数保持一致。例如我们想按迭代次数输出日志，就需要令 `log_processor` 和 `train_cfg` 的 `by_epoch=False`。
