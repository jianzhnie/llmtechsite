...
1. **单控制器架构** + 并行Worker的抽象，简化训练流程开发；
2. **并行策略与数据传输模块**，实现高效可扩展训练；
3. **Rollout 调度器**，精细控制每个样本的生命周期；
4. **环境Worker与奖励Worker**，支持智能体 RL 算法和奖励设计的快速实验；
...
- 我们在单控制器架构[^37] 的基础上，引入了明确定义的 **并行Worker（Parallel Worker）** 抽象，从而实现灵活、模块化的 RL 训练 pipeline，简化新想法的实验过程。
...
- 我们专门设计了 **环境Worker（Environment Worker）** 和 **奖励Worker（Reward Worker）**，以提供高效且可扩展的智能体环境交互和奖励计算能力。
- 我们实现了 **资源池（Resource Pool）**，并利用 **AutoDeviceMapping** 以实现高效的Worker部署和优化的资源分配。
...
#### 4.1.2 Parallel Worker（并行Worker）
