...
- **Actual FLOPs (实际达到的 FLOPs)**：指模型执行前向传播（Forward）和反向传播（Backward）及优化器更新等所需的所有计算。
...
> $$2bsh^2 + \frac{2bsh^2}{r} + \frac{2bsh^2}{r} + 2bs^2h + 2bs^2h + 2bsh^2 = 4bsh^2 + \frac{4bsh^2}{r} + 4bs^2h$$
...
- $\frac{1}{r} \approx \frac{n}{n_{kv} }$，常忽略或合并

> ✅ 最终简化公式：
> $$\text{FLOPs} \approx 12Lbsh^2 \left(1 + \frac{1}{r} + 4m + \frac{s}{h} + \frac{v}{2Lh}\right)$$
...
> 🔸 近似可得：
> $$\text{FLOPs} \approx 6bs \times (\text{总参数量} + 2Lsh)$$
