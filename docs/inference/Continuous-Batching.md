...
以下是 LLM 推理中静态批处理的示意图：

![02](https://hfai-static.high-flyer.cn/static/28d7172c5c27e843bc31bb91aacac16c/a6d36/02.png)

如上图所示，在第一遍迭代（左）中，每个序列从提示词（黄）中生成一个 Token（蓝色）。经过几轮迭代（右）后，完成的序列具有不同的尺寸，因为每个序列在不同的迭代结束时产生不同的结束序列 Token（红色）。尽管序列 3 在两次迭代后完成，但静态批处理意味着 GPU 将等待直到批处理中的最后一个序列完成。

与传统深度学习模型不同，LLM 的批处理方法比较复杂，因为其推理是迭代性的。直观上，这是因为请求可能提前完成，但释放其资源并添加可能处于不同完成状态的新请求是棘手的。这意味着，当批次中不同序列的生成长度与最大生成长度不同时，GPU 未被充分利用。在上面的右图中，这通过序列 1、3 和 4 的结束序列 Token 后的白色方块来说明。
...
采用连续批处理生成七个序列的示意图，[图片来源](https://www.anyscale.com/blog/continuous-batching-llm-inference)。在首次迭代时（左图），每个序列根据其提示词生成首个 Token。左图显示了单个迭代后的 Batch，随时间推移（右图），右图显示了多次迭代后的 Batch。一旦某个序列产生结束序列 Token，我们在其位置插入新的序列（即序列 S5、S6 和 S7）。这实现了更高的 GPU 利用率，因为 GPU 不需要等待所有序列完成才开始新的一个。



该技术采用迭代级调度策略，意味着批次组成会在每个解码迭代周期动态变化。当批次中某个序列完成 Token 生成后，服务器会立即在其位置插入新请求。这种方式通过避免因等待批次中最慢序列而产生的空闲时间，最大化 GPU 占用率并保持计算资源持续忙碌。
